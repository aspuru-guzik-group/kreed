...
def visualize_schedules():
    ...
    schedules = {
        "polynomial_1": FixedNoiseSchedule("polynomial_1", timesteps=T, s_poly=1e-5),
        "polynomial_2": FixedNoiseSchedule("polynomial_2", timesteps=T, s_poly=1e-5),
        "cosine": FixedNoiseSchedule("cosine", timesteps=T),
    ...
    for _ in range(2000):
        nn.zero_grad()
        loss = F.mse_loss(nn.sweep(T + 1)[1], gt.sweep(T + 1)[1])
        loss.backward()
        optim.step()
